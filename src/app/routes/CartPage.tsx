...
import { Button, Container, EmptyState, PageHeader, Skeleton } from '@/shared/ui'
            ...
            <div className="flex flex-col gap-3">
              {lines.map((line) => (
                <CartItemRow
                  ...
                  onQuantityChange={(quantity) => void setQuantity(line.artwork.id, quantity)}
                  onRemove={() => void removeItem(line.artwork.id)}
                />
              ))}
              {unavailableCount > 0 && (
                <p className="text-center text-sm text-text-muted">
                  {unavailableCount} cart item{unavailableCount === 1 ? ' is' : 's are'} no longer available and{' '}
